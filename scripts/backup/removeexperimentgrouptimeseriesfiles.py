...
if (arguments != 8):
    print("Error: Usage removeexperimentgrouptimeseries10yrfiles.py experimentgroup historyfiletype experimentgrouphistorylist experiment experimentlist variablelist startyear endyear")
    sys.exit()
else:
...
for requestedperiodindex in range(numrequestedperiods):
    ...
    if (os.path.isfile(globalfilename)):
        print("Removing File: " + globalfilename)
    ...
    if (variableprocesstypes[variablelistindex] == "TimeSeries"):
        ...
        for requestedperiodindex in range(numrequestedperiods):
            ...
            if (os.path.isfile(createdfilename)):
                print("Removing File: " + createdfilename)
                os.remove(createdfilename)
            if (os.path.isfile(updatingfilename)):
                print("Removing File: " + updatingfilename)
                os.remove(updatingfilename)
            if (os.path.isfile(finalfilename)):
                print("Removing File: " + finalfilename)
                os.remove(finalfilename)

        submissionfilename = gf.create_submission_filename(variabletimeseriesname,requestedstartyear,requestedendyear)
        if (os.path.isfile(submissionfilename)):
            print("Removing File: " + submissionfilename)
            os.remove(submissionfilename)

        logfilename = gf.create_log_filename(variabletimeseriesname,requestedstartyear,requestedendyear)
        if (os.path.isfile(logfilename)):
            print("Removing File: " + logfilename)
