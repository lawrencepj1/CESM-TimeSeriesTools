...
if (arguments != 12):
    print("Error: Usage submitupdateexperimentgrouptimeseries10yrfiles.py experimentgroup historyfiletype experimentgrouphistorylist experiment experimentlist variablelist startyear endyear updateexperimentgroupcommand submissions queuename projectcode")
    sys.exit()
else:
...
updateexperimentgroupcommand = str(sys.argv[9])
totalsubmissions = int(sys.argv[10])
queuename = str(sys.argv[11])
projectcode = str(sys.argv[12])
...
experimentlogfileslocation = gf.experimentgrouphistoryoutputdir + "/" + gf.experimentlongname + "/" + gf.experimentgrouphistoryoutput + "/logfiles/"
if not os.path.isdir(experimentlogfileslocation):
    os.makedirs(experimentlogfileslocation)

experimentsubmissionfileslocation = gf.experimentgrouphistoryoutputdir + "/" + gf.experimentlongname + "/" + gf.experimentgrouphistoryoutput + "/submissionfiles/"
...
for variablelistindex in range(numvariables):
    if (variableprocesstypes[variablelistindex] == "TimeSeries"):
        ...
        firstperiodindex = requestedstartperiod
        lastperiodindex = requestedendperiod
        firsttimeseriesstartyear = experimentstartyears[firstperiodindex]
        firsttimeseriesendyear = experimentendyears[firstperiodindex]
        lasttimeseriesendyear = experimentendyears[lastperiodindex]
        createdfilename = gf.create_timeseries_filename(variabletimeseriesname,firsttimeseriesstartyear,firsttimeseriesendyear,"_created")
        if (os.path.isfile(createdfilename) and submissioncount < totalsubmissions):
            ...
            submissionfilename = gf.create_submission_filename(variabletimeseriesname,firsttimeseriesstartyear,lasttimeseriesendyear)
            logfilename = gf.create_log_filename(variabletimeseriesname,firsttimeseriesstartyear,lasttimeseriesendyear)
            ...
            submissionfile.write("#PBS -A " + projectcode + " \n")
            submissionfile.write("#PBS -j oe \n")
            submissionfile.write("#PBS -o " + logfilename + " \n")
            submissionfile.write("#PBS -e " + logfilename + " \n")
            submissionfile.write("#PBS -q " + queuename + " \n")
            submissionfile.write("#PBS -l walltime=04:00:00 \n")
            ...
            submissionfile.write(" \n")
            submissionfile.write(updateexperimentgroupcommand + " " + experimentgroupname + " " + historyfiletype + " " + experimentname + " " + variabletimeseriesname + " " + requestedstartyearname + " " + requestedendyearname + " \n")
...
if (submissioncount < totalsubmissions):
    print("No More " + gf.experimentlongname + " " + gf.experimentgrouphistorytimetype + " Variables to Submit for Update from " + requestedstartyearname + " to " + requestedendyearname)
