...
if (arguments != 11):
    print("Error: Usage submitupdatecamh0timeseries10yrfiles.py experimentgroup historyfiletype experimentgrouphistorylist experiment experimentlist variablelist startyear endyear updatecamh0command submissions projectcode")
    sys.exit()
else:
...
updatecamh0command = str(sys.argv[9])
...
for variablelistindex in range(numvariables):
    if (variableprocesstypes[variablelistindex] == "TimeSeries"):
        ...
        createdfilename = experimentoutputlocation + experimentlongname + ".cam.h0." + variabletimeseriesname + "_created." + str(startyears[periodindex]).zfill(4) + "01-" + str(endyears[periodindex]).zfill(4) + "12.nc"
        logfilename = experimentoutputlocation + experimentlongname + ".cam.h0." + variabletimeseriesname + "_updating." + str(startyears[periodindex]).zfill(4) + "01-" + str(endyears[periodindex]).zfill(4) + "12.log"
        if (os.path.isfile(createdfilename) and submissioncount < totalsubmissions):
            ...
            for requestedperiodindex in range(numrequestedperiods):
                ...
                createdfilename = experimentoutputlocation + experimentlongname + ".cam.h0." + variabletimeseriesname + "_created." + str(startyears[periodindex]).zfill(4) + "01-" + str(endyears[periodindex]).zfill(4) + "12.nc"
                updatingfilename = experimentoutputlocation + experimentlongname + ".cam.h0." + variabletimeseriesname + "_updating." + str(startyears[periodindex]).zfill(4) + "01-" + str(endyears[periodindex]).zfill(4) + "12.nc"
                os.rename(createdfilename,updatingfilename)
            subprocesscommand = "qcmd -A " + projectcode + " -l select=1:ncpus=1:mem=55GB -l walltime=10:00:00 -- " + updatecamh0command + " " + experimentgroupname + " " + experimentshortname + " " + variabletimeseriesname + " " + requestedstartyearname + " " + requestedendyearname 
            logfile = open(logfilename, "w")
            subprocess.Popen(subprocesscommand.split(),stdout=logfile)
            submissioncount += 1

if (submissioncount < totalsubmissions):
    print("All " + experimentlongname + " cam.h0 Variables Submitted for Update")
