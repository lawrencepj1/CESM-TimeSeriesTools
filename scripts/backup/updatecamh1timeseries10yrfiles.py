...
if (arguments != 9):
    print("Error: Usage updatecamh1timeseries10yrfiles.py experimentgroup historyfiletype experimentgrouphistorylist experiment experimentlist variablename variablelist requestedstartyear requestedendyear")
    sys.exit()
else:
...
for variablelistindex in range(numvariables):
    if (variableprocesstypes[variablelistindex] == "TimeSeries" and variablenames[variablelistindex] == variableupdatename):
        ...
        for requestedperiodindex in range(numrequestedperiods):
            ...
            outputtempname = experimentoutputlocation + experimentlongname + ".cam.h1." + variabletimeseriesname + "_updating." + str(startyears[periodindex]).zfill(4) + "0101-" + str(endyears[periodindex]).zfill(4) + "1231.nc"
            outputfilename = experimentoutputlocation + experimentlongname + ".cam.h1." + variabletimeseriesname + "." + str(startyears[periodindex]).zfill(4) + "0101-" + str(endyears[periodindex]).zfill(4) + "1231.nc"
            ...
            for yearindex in range(numyears[periodindex]):
                ...
                for historyfileindex in range(historyfilesperyear):
                    ...
                    print("Reading: " + variabletimeseriesname + " From: " + day5experimentfilename)

                    day5experimentfile = netcdf4.Dataset(day5experimentfilename,'r')

                    readvariable = day5experimentfile.variables[variabletimeseriesname]
