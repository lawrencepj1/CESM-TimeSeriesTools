...
if (arguments != 8):
    print("Error: Usage removecamh0timeseries10yrfiles.py experimentgroup historyfiletype experimentgrouphistorylist experiment experimentlist variablelist startyear endyear")
    sys.exit()
else:
...
print("Removing Variable: GLOBAL")
...
for variablelistindex in range(numvariables):
    if (variableprocesstypes[variablelistindex] == "TimeSeries"):
        ...
        print("Removing Variable: " + variabletimeseriesname)
        for requestedperiodindex in range(numrequestedperiods):
            periodindex = requestedstartperiod + requestedperiodindex
            ...
            logfilename = experimentoutputlocation + experimentlongname + ".cam.h0." + variabletimeseriesname + "_updating." + str(startyears[periodindex]).zfill(4) + "01-" + str(endyears[periodindex]).zfill(4) + "12.nc"
            finalfilename = experimentoutputlocation + experimentlongname + ".cam.h0." + variabletimeseriesname + "." + str(startyears[periodindex]).zfill(4) + "01-" + str(endyears[periodindex]).zfill(4) + "12.nc"
            if (os.path.isfile(createdfilename)):
                os.remove(createdfilename)
            if (os.path.isfile(updatingfilename)):
                os.remove(updatingfilename)
