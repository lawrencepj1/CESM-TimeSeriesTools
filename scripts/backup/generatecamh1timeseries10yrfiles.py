...
experimentfilename = experimentfilename + "." + str(startyears[requestedstartperiod]).zfill(4) + "-01-01-00000.nc"
...
for requestedperiodindex in range(numrequestedperiods):
    ...
    outputfilename = experimentoutputlocation + experimentlongname + ".cam.h1.GLOBAL." + str(startyears[periodindex]).zfill(4) + "0101-" + str(endyears[periodindex]).zfill(4) + "1231.nc"
    ...
    periodnumdays = numdays[periodindex]
    outputfile.createDimension('time',int(periodnumdays))
    ...
    writetime[:] = outtimes[periodindex][0:periodnumdays]
    ...
    for yearindex in range(numyears[periodindex]):
        ...
        for historyfileindex in range(historyfilesperyear):
            outputindex = yearindex * 365 + historyfileindex * 5
            processmonth = historyfilemonths[historyfileindex]
            processday = historyfiledays[historyfileindex]

            day5experimentfilename = experimentlocation + experimentlongname + "/" + experimentgrouphistorysource + "/" + experimentlongname + "." + experimentgrouphistoryfiletype
            day5experimentfilename = day5experimentfilename + "." + str(processyear).zfill(4) + "-" + str(processmonth).zfill(2) + "-" + str(processday).zfill(2) +"-00000.nc"

            print("Reading File: " + day5experimentfilename)

            day5experimentfile = netcdf4.Dataset(day5experimentfilename,'r')

            for variablelistindex in range(numvariables):
                if (variableprocesstypes[variablelistindex] == "Global"):

                    variableaddname = variablesnames[variablelistindex]
                    readvariable = day5experimentfile.variables[variableaddname]
                    writevariable = outputfile.variables[variableaddname]
                    if (len(writevariable.dimensions) == 1):
                        for historytimeindex in range(historyfilenumdays):
            ...
            for timeindex in range(numvariables):
                if (variableprocesstypes[timeindex] == "Time"):
                    ...
                    timeaddname = variablesnames[timeindex]
                    readvariable = day5experimentfile.variables[timeaddname]
                    writevariable = outputfile.variables[timeaddname]
                    if (len(writevariable.dimensions) == 1):
                        for historytimeindex in range(historyfilenumdays):
                            writevariable[outputindex+historytimeindex] = readvariable[historytimeindex]
    ...
    globalfilename = experimentoutputlocation + experimentlongname + ".cam.h1.GLOBAL." + str(startyears[periodindex]).zfill(4) + "0101-" + str(endyears[periodindex]).zfill(4) + "1231.nc"
    ...
    for variablelistindex in range(numvariables):
        if (variableprocesstypes[variablelistindex] == "TimeSeries"):
            ...
            outputfilename = experimentoutputlocation + experimentlongname + ".cam.h1." + variabletimeseriesname + "_created." + str(startyears[periodindex]).zfill(4) + "0101-" + str(endyears[periodindex]).zfill(4) + "1231.nc"
            ...
            outputfile.createDimension('time',int(numdays[periodindex]))
